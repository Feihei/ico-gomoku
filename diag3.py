"""验证带 visited 的 extend 是否阻止了重复路径"""
import math
...
def extend(board_occ, adj, pos, start, came_from, player):
    line = []
    visited = set([came_from])  # 关键：阻止绕回 cameFrom
    cur = start
    prev = came_from
...
for v in range(5):
    ...
    print(f"  No win (correct)")

# Debug: what does extend return for place 0,1,2,3,4 at vertex 0, axis (2, 105)?
print("\n=== Debug: extend for place 0,1,2,3,4 ===")
board_occ4 = [None]*V
for i in range(5):
    board_occ4[i] = 1

# Vertex 0 is a 5-vertex
v = 0
axes0 = get_axes(adj, v, pos)
print(f"Vertex 0 axes: {axes0}")
for a, b in axes0:
    fwd = extend(board_occ4, adj, pos, a, v, 1)
    bwd = extend(board_occ4, adj, pos, b, v, 1)
    line = fwd + [v] + bwd
    print(f"  axis ({a},{b}): fwd={fwd}, bwd={bwd}, line={line}, len={len(line)}")
