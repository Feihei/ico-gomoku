...
def gen_icosphere(n):
    ...
    V = len(key_to_idx)
    pos = [None]*V
    for k,(ix,iy,iz) in key_to_idx.items():
        pos[ix],pos[iy],pos[iz] = float(k[0]),float(k[1]),float(k[2])
...
# Test: can we place 0,1,2,3,4 and get a win?
print("\n=== Testing place 0,1,2,3,4 ===")
board_occ = [None]*V
for i in range(5):
    board_occ[i] = 1
    # Check if any of 0..i forms a win
    for v in range(i+1):
...
for v in range(V):
    if len(adj[v]) == 6:
        ...
        # Walk from a through v (came_from = a, start = v)
        # Actually extend goes from a with cameFrom=v
        # So we walk from a, cameFrom=v, straightest direction
        path_a = []
        cur,prev = a,v
        for _ in range(10):
            best,best_ang = None,-1
            ...
            path_a.append(best)
            prev,cur = cur,best
        print(f"  From a={a} away from v: {path_a[:5]}")
        
        # Walk from b through v
        path_b = []
        cur,prev = axes[0][1],v
        for _ in range(10):
            best,best_ang = None,-1
